...
//  mnist-test
//
//  Created by Alex Chan on 9/30/18.
//  Copyright © 2018 big.dick.energy. All rights reserved.
//
...
class VisionObjectRecognitionViewController: VisionViewController {
    ...
    func drawVisionRequestResults(_ results: [Any]) {
        ...
        // print(results)
        var highestConfidence: Float = -1
        var classif: String = "-1"
        for observation in results where observation is VNClassificationObservation {
            guard let classification = observation as? VNClassificationObservation else {
                continue
            }
            
            if (Float(classification.confidence) > highestConfidence) {
                highestConfidence = classification.confidence
                classif = classification.identifier
            }
...
//        let textLayer = self.createTextSubLayerInBounds(bounds, identifier: classif, confidence: highestConfidence)
//        rootLayer.addSublayer(textLayer)
        rootLayer.addSublayer(label.layer)
